...
void init_device_id( DeviceIdentification& dev_id )
{
    ...
    // Leaving the 'name' field empty to save storage.
}
...
bool connect_to_controller()
{
    ...
    msg.which_msg_type = Msg_connect_msg_tag;
    msg.msg_type.connect_msg = Connect_init_zero;
    init_device_id( msg.msg_type.connect_msg.dev_id );
...
{
    Msg msg = Msg_init_zero;
    msg.which_msg_type = Msg_report_msg_tag;
    msg.msg_type.report_msg = Report_init_zero;
    init_device_id( msg.msg_type.report_msg.dev_id );

    msg.msg_type.report_msg.data.temperature = int32_t(last_temp_reading * 10.0);
    msg.msg_type.report_msg.data.humidity = int32_t(last_hum_reading * 10.0);
...
void loop()
{
    ...
    if ( pkt_size )
    {
        ...
        if ( status )
        {
            if ( msg.which_msg_type == Msg_command_msg_tag )
            {
                if ( msg.msg_type.command_msg.cmd_type == Command_CommandType_REPORT_DATA )
                    send_report();
                else
